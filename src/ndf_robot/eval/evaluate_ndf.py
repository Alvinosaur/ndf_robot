...
def main(args, global_dict):
    if args.debug:
        ...
        set_log_level('info')
    pb_cfg = {'gui': args.pybullet_viz,
              'realtime': True, 'opengl_render': args.pybullet_viz}
    robot = Robot('franka', pb_cfg=pb_cfg, arm_cfg={
    ...
    if args.dgcnn:
        model = vnn_occupancy_network.VNNOccNet(
            ...
            sigmoid=True,
            acts=args.acts).to(util.DEVICE)
    else:
        model = vnn_occupancy_network.VNNOccNet(
            latent_dim=256,
            model_type='pointnet',
            return_features=True,
            sigmoid=True).to(util.DEVICE)
    ...
    for i, fname in enumerate(grasp_demo_filenames):
        ...
        # Available info: ['shapenet_id', 'ee_pose_world', 'robot_joints', 'obj_pose_world', 'obj_pose_camera', 'object_pointcloud', 'depth', 'seg', 'camera_poses', 'obj_model_file', 'obj_model_file_dec', 'gripper_pts', 'gripper_pts_gaussian', 'gripper_pts_uniform', 'gripper_contact_pose', 'table_urdf']
