...
def get_graph_feature(x, k=20, idx=None, x_coord=None):
    ...
    idx_base = torch.arange(
        0, batch_size, device=util.DEVICE).view(-1, 1, 1) * num_points
    ...
    feature = torch.cat((feature - x, x), dim=3).permute(0,
                                                         3, 4, 1, 2).contiguous()
...
def get_graph_feature_cross(x, k=20, idx=None):
    """
    Predefined mapping of PC to PC features that are fed into Encoder.
    Map x -> [feature - x, x, cross] 
    """
    batch_size = x.size(0)
    num_points = x.size(3)
    x = x.view(batch_size, -1, num_points)
    ...
    # offset each batch by its batch_idx * num_points_per_batch
    idx_base = torch.arange(
        0, batch_size, device=util.DEVICE).view(-1, 1, 1) * num_points
    ...
    # (batch_size, num_points, num_dims)  -> (batch_size*num_points, num_dims) #   batch_size * num_points * k + range(0, batch_size*num_points)
    x = x.transpose(2, 1).contiguous()
    # for each point cloud, get the k nearest neighbors of the point cloud
    # and use this to represent as a "feature" of B x num_points x k x num_dims x 3
    feature = x.view(batch_size * num_points, -1)[idx, :]
    feature = feature.view(batch_size, num_points, k, num_dims, 3)
    x = x.view(batch_size, num_points, 1, num_dims, 3).repeat(1, 1, k, 1, 1)

    # ipdb> x.shape
    # torch.Size([32, 1000, 20, 1, 3]) each of the 1000 pts repeated k times
    # ipdb> feature.shape
    # torch.Size([32, 1000, 20, 1, 3]) each of the 1000 pts' k nearest neighbors
    cross = torch.cross(feature, x, dim=-1)

    # cat dim=3 concatenates each of these three feats [feat - x, x, cross]
    # (B x N x K x D x 3) -> (B x D x 3 x N x K)
...
def get_graph_mean(x, k=20, idx=None):
    ...
    feature = feature.view(batch_size, num_points, k,
                           num_dims, 3).mean(2, keepdim=False)
...
def get_shell_mean_cross(x, k=10, nk=4, idx_all=None):
    ...
    for i in range(nk):
        ...
        feature[i] = feature[i].view(
            batch_size, num_points, k, num_dims, 3).mean(2, keepdim=False)
...
class VNResnetBlockFC(nn.Module):
    ''' Fully connected ResNet Block class. (x -> x + f(x))
    ...
    def __init__(self, size_in, size_out=None, size_h=None):
        ...
        self.actvn_1 = VNLeakyReLU(
            size_h, negative_slope=0.2, share_nonlinearity=False)
    ...
    def forward(self, x):
        # can't use standard sequential even though activations are the same
        # type bc those are VN RELUs which have learnable direction
