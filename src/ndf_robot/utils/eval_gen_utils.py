...
def soft_grasp_close(robot, joint_id2, force=100):
    p.setJointMotorControl2(robot.arm.robot_id, joint_id2,
                            p.VELOCITY_CONTROL, targetVelocity=-1, force=force)
...
def safeCollisionFilterPair(bodyUniqueIdA, bodyUniqueIdB, linkIndexA, linkIndexB, enableCollision, *args, **kwargs):
    if bodyUniqueIdA is not None and bodyUniqueIdB is not None and linkIndexA is not None and linkIndexB is not None:
        p.setCollisionFilterPair(bodyUniqueIdA=bodyUniqueIdA, bodyUniqueIdB=bodyUniqueIdB,
                                 linkIndexA=linkIndexA, linkIndexB=linkIndexB, enableCollision=enableCollision)
...
def object_is_still_grasped(robot, obj_id, right_pad_id, left_pad_id):
    ...
    obj_still_in_grasp = len(obj_finger_left_info) > 0 or len(
        obj_finger_right_info) > 0
...
def process_demo_data_rack(grasp_data, place_data, cfg, gaussian_scale=0.1):
    ...
    inliers = np.where(np.linalg.norm(
        demo_obj_pts - demo_pts_mean, 2, 1) < 0.2)[0]
    demo_obj_pts = demo_obj_pts[inliers]

    # points we use to represent the gripper at their canonical pose position shown in the demonstration
    demo_gripper_pts_rs = data['gripper_pts']
    demo_gripper_pcd_rs = trimesh.PointCloud(demo_gripper_pts_rs)
    demo_ee_mat = util.matrix_from_pose(util.list2pose_stamped(
        data['ee_pose_world']))  # end-effector pose before grasping
    demo_gripper_pcd_rs.apply_transform(demo_ee_mat)
    # points we use to represent the gripper at their canonical pose position shown in the demonstration
    demo_gripper_pts_rs = np.asarray(demo_gripper_pcd_rs.vertices)

    # demo_gripper_pts = data['gripper_pts_gaussian'] * gaussian_scale # query points for the gripper (Gaussian distributed)
    # query points for the gripper (Uniform distributed)
    ...
    # observed shape points at start
    place_demo_obj_pts = place_data['object_pointcloud']
    place_demo_pts_mean = np.mean(place_demo_obj_pts, axis=0)
    inliers = np.where(np.linalg.norm(place_demo_obj_pts -
                       place_demo_pts_mean, 2, 1) < 0.2)[0]
    ...
    # start shape points transformed into goal configuration
    place_demo_obj_pcd.apply_transform(place_demo_obj_pose_rel_mat)
    place_demo_obj_pts = np.asarray(
        place_demo_obj_pcd.vertices)  # shape points at goal

    # points used to represent the rack in canonical pose
    place_demo_rack_pts_rs = place_data['rack_pointcloud_gt']
    place_demo_rack_pcd_rs = trimesh.PointCloud(place_demo_rack_pts_rs)
    place_demo_rack_pose_mat = util.matrix_from_pose(
        util.list2pose_stamped(place_data['rack_pose_world']))
    # points used to represent the rack in demo pose
    place_demo_rack_pcd_rs.apply_transform(place_demo_rack_pose_mat)
    place_demo_rack_pts_rs = np.asarray(place_demo_rack_pcd_rs.vertices)

    place_demo_rack_pts_obs = np.concatenate(
        place_data['rack_pointcloud_observed'], 0)  # points that we observed on the rack
    rndperm = np.random.permutation(place_demo_rack_pts_obs.shape[0])
    place_demo_rack_pts_obs = place_demo_rack_pts_obs[rndperm[:int(
        place_demo_rack_pts_obs.shape[0] / 2)]]

    # points used to represent the rack in canonical pose
    gaussian_place_demo_rack_pts = place_data['rack_pointcloud_gaussian'] * gaussian_scale
    gaussian_place_demo_rack_pcd = trimesh.PointCloud(
        gaussian_place_demo_rack_pts)
    gaussian_place_demo_rack_pose_mat = util.matrix_from_pose(
        util.list2pose_stamped(place_data['rack_contact_pose']))
    # points used to represent the rack in demo pose
    gaussian_place_demo_rack_pcd.apply_transform(
        gaussian_place_demo_rack_pose_mat)
    gaussian_place_demo_rack_pts = np.asarray(
        gaussian_place_demo_rack_pcd.vertices)

    uniform_place_demo_rack_pts = place_data['rack_pointcloud_uniform']
    uniform_place_demo_rack_pcd = trimesh.PointCloud(
        uniform_place_demo_rack_pts)
    uniform_place_demo_rack_pose_mat = util.matrix_from_pose(
        util.list2pose_stamped(place_data['rack_pose_world']))
    # points used to represent the rack in demo pose
    uniform_place_demo_rack_pcd.apply_transform(
        uniform_place_demo_rack_pose_mat)
    uniform_place_demo_rack_pts = np.asarray(
        uniform_place_demo_rack_pcd.vertices)
...
def process_demo_data_shelf(grasp_data, place_data, cfg):
    """
    1. Filters out outlier points from object point cloud
    2. Applies end-effector pose gripper points of demonstration
    3. Same as step 2, but also applied to gripp points sampled uniformly???
    """
    data = grasp_data
    # observed shape point cloud at start
    demo_obj_pts = data['object_pointcloud']
    ...
    # points used to represent the rack in canonical pose
    place_demo_shelf_pts_rs = place_data['shelf_pointcloud_gt']
    place_demo_shelf_pcd_rs = trimesh.PointCloud(place_demo_shelf_pts_rs)
    place_demo_shelf_pose_mat = util.matrix_from_pose(
        util.list2pose_stamped(place_data['shelf_pose_world']))
    # points used to represent the rack in demo pose
    place_demo_shelf_pcd_rs.apply_transform(place_demo_shelf_pose_mat)
    place_demo_shelf_pts_rs = np.asarray(place_demo_shelf_pcd_rs.vertices)

    place_demo_rack_pts_obs = np.concatenate(
        place_data['shelf_pointcloud_observed'], 0)  # points that we observed on the rack
    rndperm = np.random.permutation(place_demo_rack_pts_obs.shape[0])
    place_demo_rack_pts_obs = place_demo_rack_pts_obs[rndperm[:int(
        place_demo_rack_pts_obs.shape[0] / 2)]]

    uniform_place_demo_shelf_pts = place_data['shelf_pointcloud_uniform']
    uniform_place_demo_shelf_pcd = trimesh.PointCloud(
        uniform_place_demo_shelf_pts)
    uniform_place_demo_shelf_pose_mat = util.matrix_from_pose(
        util.list2pose_stamped(place_data['shelf_pose_world']))
    # points used to represent the rack in demo pose
    uniform_place_demo_shelf_pcd.apply_transform(
        uniform_place_demo_shelf_pose_mat)
    uniform_place_demo_shelf_pts = np.asarray(
        uniform_place_demo_shelf_pcd.vertices)
...
def process_xq_data(grasp_data, place_data, shelf=True):
    """
    Loads either rack or shelf point cloud. Extracts pose of rack/shelf
    from place data and applies to point cloud. Overall returns 
        - grasp points
        - transformed rack/shelf points
...
def post_process_grasp_point(pre_grasp_ee_pose, target_obj_pcd, thin_feature=True, grasp_viz=False, grasp_dist_thresh=0.0025):
    ...
    target_obj_voxel_down = target_obj_pcd[rix[:int(
        target_obj_pcd.shape[0] / 5)]]
    ...
    while True:
        ...
        new_idxs_within_ball = target_obj_down_tree.query_ball_point(
            new_grasp_pt, r=0.015)
        new_idxs_within_larger_ball = target_obj_down_tree.query_ball_point(
            new_grasp_pt, r=0.02)
    ...
    local_grasp_normal = util.vec_from_pose(
        util.list2pose_stamped(pre_grasp_ee_pose))[1]
    ...
    if grasp_viz:
        ...
        grasp_sph.visual.face_colors = np.tile(
            [40, 40, 40, 255], (grasp_sph.faces.shape[0], 1))
        grasp_sph.apply_translation(grasp_pt)

        new_grasp_sph = trimesh.creation.uv_sphere(0.005)
        new_grasp_sph.visual.face_colors = np.tile(
            [40, 255, 40, 255], (new_grasp_sph.faces.shape[0], 1))
